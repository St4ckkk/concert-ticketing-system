...
$concertId = isset($_SESSION['concert_id']) ? $_SESSION['concert_id'] : null;
$user_id = isset($_SESSION['user_id']) ? $_SESSION['user_id'] : null;

if ($concertId === null || $user_id === null) {
    die('Concert ID or User ID is not set in the session.');
}
...
$transactionNumber = ''; // Initialize the variable
?>
...
<body>
    <div class="form-container">
            ...
            <image src="css/images/concertmap.png" id="seatimage">
                <div class="wrapper">
                    ...
                    <label for="text1" class="labeltext" id="paymentmode">Payment Mode:
                        <?php echo $selectedPayment; ?></label>

                    <label for="text1" class="labeltext" id="labeltext">Chosen Seats:
                        <?php echo $selectedSeats; ?></label>

                    <p id="total">Total Amount: <span id="result"><?php echo number_format($totalPrice, 2); ?></span>
                    </p>
        ...
        if (isset($_POST['pinconfirm'])) {
            try {
                if (!is_null($selectedSeatIds) && !empty($selectedSeatIds)) {
                    $Cashmsg = " $selectedSeatsCount seats were successfully reserved.";
                    $Cash = true;
        ...
        <?php if (!empty($Cashmsg)): ?>
            <p class="pin-message"><?php echo $Cashmsg; ?></p>
        <?php endif; ?>
        <div>
            ...
            <input type="button" name="paymentcash" class="form-btn" id="paymentconfirmcash"
                ...
                onclick="redirectToAnotherPageCash('<?php echo $concertId ?>', '<?php echo $selectedSeats; ?>', '<?php echo $totalPrice; ?>', '<?php echo $selectedSeatsCount; ?>', '<?php echo $transactionNumber; ?>')">
</body>
...
<script>
    ...
    function redirectToAnotherPageCash(concertId, seats, quantity, seatscount, transactionNumber) {
        ...
        if (Cash) {
            var url = 'paymentinfocash.php?concert_id=' + concertId + '&selected_seats=' + seats + '&total_price=' + quantity + '&count=' + seatscount + '&transaction_no=' + transactionNumber;
